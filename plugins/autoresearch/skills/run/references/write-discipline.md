...
| `.autoresearch/{expr}/runs/{run_id}/wandb_pointer.json` | Read-only | Written by AR-SAVE before final metric log. `ar` uses to bind the correct wandb run under DDP. |
...
- **train.py writing to `runs/{run_id}/` artifacts (state.pt, wandb_pointer.json)** — allowed inside the frozen AR-SAVE block. The agent does not edit the AR-SAVE block; it is a template-frozen region.
- **Subprocess-level wandb writes into `wandb/run-*/`** — allowed; that's the training run's normal output.
